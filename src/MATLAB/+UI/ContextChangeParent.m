% ContextChangeParent.m - Function for context menu call back for changing
% parent
...
try
    ...
    Tracks.ChangeTrackParent(newParentID,time,trackID);
catch errorMessage
    try
        Error.ErrorHandeling(['ChangeTrackParent(' num2str(newParentID) ' ' num2str(time) ' ' num2str(trackID) ') -- ' errorMessage.message],errorMessage.stack);
        return
    catch errorMessage2
...
Error.LogAction(['Changed parent of ' num2str(trackID)],oldParent,newParentID);

UI.DrawTree(CellTracks(newParentID).familyID);
UI.DrawCells();
end

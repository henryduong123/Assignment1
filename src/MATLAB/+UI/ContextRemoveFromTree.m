...
try
    ...
    if(~isempty(index))
        droppedTracks = Tracks.ChangeLabel([],sibling,oldParent);
    end
    UI.History('Push');
catch errorMessage
    try
        Error.ErrorHandeling(['RemoveFromTree(' num2str(time) ' ' num2str(trackID) ' yes) -- ' errorMessage.message],errorMessage.stack);
        return
    catch errorMessage2
...
Families.ProcessNewborns(1:length(CellFamilies));

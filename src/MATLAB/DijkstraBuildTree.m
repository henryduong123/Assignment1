function DijkstraBuildTree(rootTracks, stopTracks)
    ...
    for i=1:length(rootTracks)
        startHulls = [startHulls GetHullID(CellTracks(rootTracks(i)).startTime, rootTracks(i))];
    end
    ...
    if ( ~exist('stopTracks','var') )
        ...
        for i=1:length(stopTracks)
            stopHulls = [stopHulls GetHullID(CellTracks(stopTracks(i)).endTime, stopTracks(i))];
        end
    end
    ...
    for i=1:length(splitTimes)
        ...
        ChangeTrackParent(parentTracks(i),CellTracks(tracks(i)).startTime,tracks(i));
    end
end
...
function newTrackID = buildTrack(path)
    ...
    startTrack = GetTrackID(path(1));
    
    if ( CellTracks(startTrack).startTime ~= startTime )
        RemoveFromTree(startTime, startTrack, 'yes');
    end
    
    for t=1:length(path)-1
        AssignEdge(path(t),path(t+1),0);
    end
    
    newTrackID = GetTrackID(path(1));
end
...
function paths = dijkstraPaths(rootHulls, stopHulls)
    ...
    for t=tStart:tEnd-1
        for i=1:length(rootHulls)
            ...
            [costMatrix bPathHulls bNextHulls] = GetCostSubmatrix(pathHulls{i}, 1:length(CellHulls));

...
if(remove)
    Families.RemoveChildren(trackID);
else
    for i=1:length(children)
        familyID = Families.NewCellFamily(children(i).hulls(1),children(i).startTime);
        newTrackID = CellFamilies(familyID).rootTrackID;
        for j=2:length(children(i).hulls)
            if(children(i).hulls(j)~=0)
                Tracks.AddHullToTrack(children(i).hulls(j),newTrackID,[]);
            end
        end
    end
end

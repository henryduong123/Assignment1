...
function GlobalPatching()
    ...
    for idx=1:length(val)
        ...
        childTrack = Tracks.GetTrackID(childHulls(c(idx)));
        parentTrack = Tracks.GetTrackID(parentHulls(r(idx)));
        ...
        parentScore = Tracker.GetTrackSegScore(parentTrack);
        childScore = Tracker.GetTrackSegScore(childTrack);
        ...
        if ( CellTracks(childTrack).startTime <= CellTracks(parentTrack).endTime )
            %TODO fix func call
            Families.RemoveFromTree(CellTracks(childTrack).startTime, parentTrack, 'no');
        end
        
        Tracks.ChangeLabel(CellTracks(childTrack).startTime, childTrack, parentTrack);%TODO change func call
        Tracks.RehashCellTracks(parentTrack,CellTracks(parentTrack).startTime);

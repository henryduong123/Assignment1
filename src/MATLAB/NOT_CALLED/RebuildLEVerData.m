...
function [assignedFromHulls,assignedToHulls] = assignToTracks(fromHulls, toHulls)
    ...
    [costMatrix, bFromHulls, bToHulls] = Tracker.GetCostSubmatrix(fromHulls, toHulls);
    ...
    for i=1:length(matchedIdx)
        ...
        trackID = Tracks.GetTrackID(fromHullID);
        Tracks.AddHullToTrack(toHullID,trackID);
...
function trackID = InitializeTrack(hullID)
    ...
    famID = Families.NewCellFamily(hullID, CellHulls(hullID).time);

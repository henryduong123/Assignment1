...
if(time > CellTracks(childTrackID).startTime)
    %TODO fix func call
    newFamilyID = Families.RemoveFromTree(time,childTrackID,'yes');
...
elseif(hash <= length(CellTracks(parentTrackID).hulls))
    ...
    siblingTrackID = Tracks.SplitTrack(parentTrackID,parentHullID); % SplitTrack adds sibling to the parent already
else
    %just rename the child to the parent
    Tracks.ChangeLabel(time,childTrackID,parentTrackID);%TODO fix function call
    return
end
...
if(oldFamilyID~=newFamilyID)
    Families.ChangeTrackAndChildrensFamily(oldFamilyID,newFamilyID,childTrackID);
end

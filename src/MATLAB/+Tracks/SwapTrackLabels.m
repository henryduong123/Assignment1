...
Hulls.MarkHullAsEdited(CellTracks(trackID1).hulls(track1Hash),time);
...
%update HashedCells
Tracks.UpdateHashedCellsTrackID(trackID1,track2Hulls,time);
Tracks.UpdateHashedCellsTrackID(trackID2,track1Hulls,time);
...
if(CellTracks(trackID1).familyID ~= CellTracks(trackID2).familyID)
    for i=1:length(CellTracks(trackID1).childrenTracks)
        Families.ChangeTrackAndChildrensFamily(CellTracks(trackID2).familyID,CellTracks(trackID1).familyID,CellTracks(trackID1).childrenTracks(i));
    end
    for i=1:length(CellTracks(trackID2).childrenTracks)
        Families.ChangeTrackAndChildrensFamily(CellTracks(trackID1).familyID,CellTracks(trackID2).familyID,CellTracks(trackID2).childrenTracks(i));
    end
end 

%check to see if either of the tracks are dead
familyIDs = [];
if(~isempty(Tracks.GetTimeOfDeath(trackID1)))
    familyIDs = Tracks.StraightenTrack(trackID1);
end
if(~isempty(Tracks.GetTimeOfDeath(trackID2)))
    familyIDs = [familyIDs Tracks.StraightenTrack(trackID2)];
end
if(~isempty(familyIDs))
    Families.ProcessNewborns(familyIDs,SegmentationEdits.maxEditedFrame);
end
end
